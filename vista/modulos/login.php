                    ...
                    <form method="post">
                        ...
                        <?php if ($mostrarCaptchaCloudflare): ?>
                            ...
                            if (
                                    ...
                                    isset($_ENV["CLOUDFLARE_LOCAL_SITE"])
                            ) {
                                    $site_key = $_ENV["CLOUDFLARE_LOCAL_SITE"];
                            } elseif (
                                    $_ENV["INSTALACION"] == "SERVIDOR" &&
                                    isset($_ENV["CLOUDFLARE_SERVIDOR_SITE"])
                            ) {
                                    $site_key =
                                            $_ENV["CLOUDFLARE_SERVIDOR_SITE"];
                            } else {
                                    // key por defecto si no hay variables de entorno configuradas
                                    $site_key = "0x4AAAAAABUTeiES0tXs0HGp";
                            }
                            ?>
                            <div class="cf-turnstile" data-sitekey="<?php echo htmlspecialchars(
                                    $site_key,
                            ); ?>"></div>
                        <?php endif; ?>
