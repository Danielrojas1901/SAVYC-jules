...
if (isset($_POST["ingresar"])) {
        ...
        if ($usarCaptchaCloudflare && isset($_POST["cf-turnstile-response"])) {
                ...
                $secret_key = "";
                if (
                        $_ENV["INSTALACION"] == "LOCAL" &&
                        isset($_ENV["CLOUDFLARE_LOCAL"])
                ) {
                        $secret_key = $_ENV["CLOUDFLARE_LOCAL"];
                } elseif (
                        $_ENV["INSTALACION"] == "SERVIDOR" &&
                        isset($_ENV["CLOUDFLARE_SERVIDOR"])
                ) {
                ...
                if (curl_errno($ch)) {
                        ...
                        $_SESSION["login"] = [
                                "title" => "Error",
                                "message" =>
                                        "Error al verificar el CAPTCHA de Cloudflare.",
                                "icon" => "error",
                        ];
        ...
        if (!empty($errores)) {
                ...
                if (
                        !empty($respuesta) &&
                        isset($respuesta["user"]) &&
                        $respuesta["status"] == 1
                ) {
                        if (
                                $respuesta["user"] == $_POST["ingUsuario"] &&
                                password_verify(
                                        $_POST["ingPassword"],
                                        $respuesta["password"],
                                )
                                ...
                                $_SESSION["cod_usuario"] =
                                        $respuesta["cod_usuario"];
                                $rol = $objRol->consultarLogin(
                                        $respuesta["cod_tipo_usuario"],
                                );
